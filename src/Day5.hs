...
day5a input = runST $ do
    mem <- parseST input
    runLast Memory { readMem = readArray mem, writeMem = writeArray mem } [1]
  where
...
day5b input = runST $ do
    mem <- parseST input
    runLast Memory { readMem = readArray mem, writeMem = writeArray mem } [5]
  where
    parseST :: String -> ST s (STUArray s Int Int)
    parseST = either (fail . show) thaw . parse (parser @UArray @Int @()) ""
